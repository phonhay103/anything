...
Type* makeDoubleType(void) { // (1)
  Type* type = (Type*) malloc(sizeof(Type));
  type->typeClass = TP_DOUBLE;
  return type;
}

Type* makeStringType(void) { // (1)
  Type* type = (Type*) malloc(sizeof(Type));
...
Type* duplicateType(Type* type) {
  ...
  if (type->typeClass == TP_ARRAY) {
    ...
    resultType->elementType = type->elementType;
  }
  return resultType;
}
...
ConstantValue* makeDoubleConstant(double d) { // (1)
  ConstantValue* value = (ConstantValue*) malloc(sizeof(ConstantValue));
...
ConstantValue* makeStringConstant(char* st) { // (1)
  ConstantValue* value = (ConstantValue*) malloc(sizeof(ConstantValue));
  value->type = TP_STRING;
  strcpy(value->stringValue, strdup(st));
  return value;
}
...
ConstantValue* duplicateConstantValue(ConstantValue* v) {
  ...
  switch (v->type) {
    case TP_INT:
      value->intValue = v->intValue; break;
    case TP_DOUBLE: // (1)
      value->doubleValue = v->doubleValue; break;
    case TP_STRING: // (1)
      strcpy(value->stringValue, v->stringValue); break;
    case TP_CHAR:
      value->charValue = v->charValue; break;
    default:
      break;
  }
  return value;
...
void initSymTab(void) {
  ...
  doubleType = makeDoubleType(); // (1)
  stringType = makeStringType(); // (1)
}
...
void cleanSymTab(void) {
  ...
  freeType(intType);
  freeType(charType);
  freeType(doubleType); // (1)
  freeType(stringType); // (1)
}
