...
// extern Type* doubleType; // (1)
// extern Type* stringType; // (1)
...
void compileAssignSt(void) { // (2)
  ...
  if (m < n)
    ...
    for (int i = 0; i < m; ++i) {
      if ((L[i]->typeClass == TP_INT) && (R[i]->typeClass == TP_DOUBLE))
        error(ERR_CONVERT_DOUBLE_INT, currentToken->lineNo, currentToken->colNo);
      else if ((L[i]->typeClass != TP_DOUBLE) && (R[i]->typeClass != TP_INT))
        checkTypeEquality(L[i], R[i]);
    }
  }
}
...
Type* compileTerm2(Type* argType1) {
  ...
  case SB_MODULUS:
    ...
    resultType = compileTerm2(type);
    if ((resultType == NULL) || (resultType->typeClass != TP_INT))
      error(ERR_INVALID_MODUL_OPERATOR, currentToken->lineNo, currentToken->colNo);
    break;
    // check the FOLLOW set
  case SB_PLUS:
